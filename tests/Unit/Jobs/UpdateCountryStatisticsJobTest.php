...
final class UpdateCountryStatisticsJobTest extends TestCase
{
    ...
    #[Test]
    public function it_activates_allergens_with_more_than_3_recipes(): void
    {
        $country = Country::factory()->create();
        ...
        $allergen->refresh();
        $this->assertTrue($allergen->active);
    }

    #[Test]
    public function it_deactivates_allergens_with_3_or_less_recipes(): void
    {
        $country = Country::factory()->create();
        $allergen = Allergen::factory()->for($country)->create(['active' => true]);
    ...
    #[Test]
    public function it_activates_ingredients_with_at_least_one_recipe(): void
    {
        $country = Country::factory()->create();
        $ingredient = Ingredient::factory()->for($country)->create(['active' => false]);
        ...
        $ingredient->refresh();
        $this->assertTrue($ingredient->active);
    }

    #[Test]
    public function it_deactivates_ingredients_with_no_recipes(): void
    {
        $country = Country::factory()->create();
        $ingredient = Ingredient::factory()->for($country)->create(['active' => true]);

        $job = new UpdateCountryStatisticsJob($country);
        $job->handle();

        $ingredient->refresh();
        $this->assertFalse($ingredient->active);
    }
